...
export async function addTemperatureEntry(
  ...
  redirectToBatch(`/batches/${data.batchRecordId}/sections/5`)
}

const CorrectTemperatureSchema = AddTemperatureSchema.extend({
  correctsEntryId: z.string().uuid(),
  correctionReason: z.string().min(1),
})

export async function correctTemperatureEntry(
  _prevState: FormActionState,
  formData: FormData,
): Promise<FormActionState> {
  const user = await verifySession()
  ...
  const parsed = CorrectTemperatureSchema.safeParse(Object.fromEntries(formData))
  if (!parsed.success) {
  ...
  const original = await prisma.temperatureEntry.findUnique({ where: { id: data.correctsEntryId } })
  if (!original || original.batchRecordId !== data.batchRecordId) {
    return { error: 'Original entry not found.' }
  }
  const alreadySuperseded = await prisma.temperatureEntry.findFirst({ where: { correctsEntryId: original.id } })
